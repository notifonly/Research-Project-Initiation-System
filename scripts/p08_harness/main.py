#!/usr/bin/env python3
"""P08 Cross-Ethnic Multi-omics Research Plan Quality Harness — Entry Point.

Thin wrapper around the shared harness core (scripts/p05_harness/).
Sets p08-specific domain prompts, config, and output paths.

Usage:
    python scripts/p08_harness/main.py                    # Full run (top 10 deep + 30 summaries)
    python scripts/p08_harness/main.py --max-candidates 5  # Only first 5 candidates
    python scripts/p08_harness/main.py --candidates T006,T004  # Specific candidates
    python scripts/p08_harness/main.py --skip-mcp          # LLM only, no literature search
    python scripts/p08_harness/main.py --merge run1,run2   # Merge saved runs
"""
...
def main():
    parser = argparse.ArgumentParser(description="P08 Cross-Ethnic Multi-omics Research Plan Quality Harness")
...
async def run_harness(args) -> None:
    ...
    output_dir = PROJECT_ROOT / config.get("output", {}).get("dir", "data/p08_harness_output")
    if args.output_dir:
...
def load_project_data(config: dict) -> tuple:
    ...
    project_id = config.get("project_id", "p08_cross_ethnic_multiomics")
