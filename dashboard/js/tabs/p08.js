const P08Tab = {
  // 评分维度（优先从 data.json dimensions 读取，回退到硬编码以对齐 scripts/p08_harness/config.yaml rubric）
  DIM_KEYS: [],
  ...
  _initDims() {
    var dims = (App.DATA.p08_research_plans && App.DATA.p08_research_plans.dimensions);
    if (dims && dims.length) {
  ...
  render(el) {
    const plans = App.DATA.p08_research_plans;
    if (!plans || !plans.candidates || !plans.candidates.length) {
      el.innerHTML = '<div class="state-empty"><div class="icon">🌍</div><h3>暂无 P08 方案质量数据</h3><p>请先运行 python scripts/p08_harness/main.py 生成研究方案验收结果</p></div>';
      return;
    }
    ...
    el.innerHTML = ''
      + '<h3 style="font-size:16px;margin-bottom:16px">🌍 P08 研究方案质量验收</h3>'
      + '<div class="stats-row">'
      ...
      + '<div class="chart-row">'
      + '<div class="chart-card full"><h3>候选方案评分概览</h3><div class="chart-box" id="chart-p08-scores" style="height:300px"></div></div>'
      + '</div>'
      + '<h3 style="margin-top:16px;margin-bottom:12px;font-size:15px">📋 深度分析候选 (' + plans.candidates.length + ' 个)</h3>'
      + '<div class="chart-card"><div class="table-wrap" id="p08-table-wrap"></div></div>';
  ...
  _renderScoreBar() {
    var c = initChart('chart-p08-scores');
    if (!c) return;
    var plans = App.DATA.p08_research_plans;
    if (!plans) return;

    var cands = plans.candidates.slice().sort(function(a, b) { return b.final_score - a.final_score; });
    var cats = cands.map(function(cd) { return cd.candidate_id.replace('p08_cross_ethnic_multiomics_', ''); });
    var dimKeys = P08Tab.DIM_KEYS;
    var dimNames = dimKeys.map(function(k) { return P08Tab.DIM_LABELS[k]; });
    var dimColors = P08Tab.DIM_COLORS;
  ...
  _renderCandidateGrid() {
    var el = document.getElementById('p08-table-wrap');
    if (!el) return;
    var plans = App.DATA.p08_research_plans;
    var cands = plans.candidates.slice().sort(function(a, b) { return b.final_score - a.final_score; });
    ...
    for (var i = 0; i < cands.length; i++) {
      var cd = cands[i];
      var shortId = cd.candidate_id.replace('p08_cross_ethnic_multiomics_', '');
      ...
      var nvBadge = nv.overall_verdict ? P08Tab._verdictBadge(nv.overall_verdict, nv.papers_found) : '';
      var rtBadge = '';
      ...
      html += '<div class="stat-card cursor" onclick="P08Tab.showDetail(' + i + ')" style="flex:1 1 300px;min-width:280px;max-width:360px;padding:14px;transition:box-shadow .2s">'
  ...
  showDetail(idx) {
    var cands = App.DATA.p08_research_plans.candidates.slice().sort(function(a, b) { return b.final_score - a.final_score; });
    var cd = cands[idx];
    if (!cd) return;
    ...
    var noveltyHTML = P08Tab._renderNoveltySection(cd);
    var redteamHTML = P08Tab._renderRedteamSection(cd);
    ...
    if (cd.critique_text) {
      ...
      if (cd.detailed_feedback && Object.keys(cd.detailed_feedback).length) {
        ...
        for (var fi = 0; fi < fbKeys.length; fi++) {
          var flabel = P08Tab.DIM_LABELS[fbKeys[fi]] || fbKeys[fi];
  ...
  _renderNoveltySection(cd) {
    ...
    html += '<div class="field" style="font-size:12px;margin-bottom:8px">'
      + '整体判定: ' + P08Tab._verdictBadge(nv.overall_verdict, nv.papers_found)
    ...
    if (nvi.overall_verdict && nvi.overall_verdict !== nv.overall_verdict) {
      html += '<div style="font-size:11px;color:var(--text-muted);margin-bottom:8px">初始方案判定: ' + P08Tab._verdictBadge(nvi.overall_verdict, nvi.papers_found) + '（refine 前）</div>';
    }
    ...
    for (var vi = 0; vi < verdicts.length; vi++) {
      ...
      html += '<details class="field" style="margin-bottom:6px"><summary style="cursor:pointer;font-size:12px">'
        + P08Tab._verdictBadge(vd.verdict) + ' <span title="' + claim.replace(/"/g, '&quot;') + '">' + claimShort + '</span>'
