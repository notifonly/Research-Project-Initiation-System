const P09Tab = {
  ...
  _initDims() {
    var dims = (App.DATA.p09_research_plans && App.DATA.p09_research_plans.dimensions);
    if (dims && dims.length) {
  ...
  render(el) {
    const plans = App.DATA.p09_research_plans;
    if (!plans || !plans.candidates || !plans.candidates.length) {
      el.innerHTML = '<div class="state-empty"><div class="icon">🧬</div><h3>暂无 P09 方案质量数据</h3><p>请先运行 python scripts/p09_harness/main.py 生成研究方案验收结果</p></div>';
      return;
    }
    ...
    el.innerHTML = ''
      + '<h3 style="font-size:16px;margin-bottom:16px">🧬 P09 scGWAS × 空间转录组网络模块发现</h3>'
      + '<p style="font-size:13px;color:var(--text-secondary);margin-bottom:16px;line-height:1.6">将 scGWAS 双权重网络模块搜索（Box-Cox 归一化 + MEBE 贪婪扩展）适配到空间转录组学微域，用空间邻域图替代 PPI 网络，在 gsMap GSS 评分上执行空间约束的模块发现。</p>'
      + '<div class="stats-row">'
      ...
      + '<div class="chart-row">'
      + '<div class="chart-card full"><h3>候选方案评分概览</h3><div class="chart-box" id="chart-p09-scores" style="height:300px"></div></div>'
      + '</div>'
      + '<h3 style="margin-top:16px;margin-bottom:12px;font-size:15px">📋 深度分析候选 (' + plans.candidates.length + ' 个)</h3>'
      + '<div class="chart-card"><div class="table-wrap" id="p09-table-wrap"></div></div>';
  ...
  _renderScoreBar() {
    var c = initChart('chart-p09-scores');
    if (!c) return;
    var plans = App.DATA.p09_research_plans;
    if (!plans) return;

    var cands = plans.candidates.slice().sort(function(a, b) { return b.final_score - a.final_score; });
    var cats = cands.map(function(cd) { return cd.candidate_id.replace('p09_spatial_gwas_network_', ''); });
    var dimKeys = P09Tab.DIM_KEYS;
    var dimNames = dimKeys.map(function(k) { return P09Tab.DIM_LABELS[k]; });
    var dimColors = P09Tab.DIM_COLORS;
  ...
  _renderCandidateGrid() {
    var el = document.getElementById('p09-table-wrap');
    if (!el) return;
    var plans = App.DATA.p09_research_plans;
    var cands = plans.candidates.slice().sort(function(a, b) { return b.final_score - a.final_score; });
    ...
    for (var i = 0; i < cands.length; i++) {
      var cd = cands[i];
      var shortId = cd.candidate_id.replace('p09_spatial_gwas_network_', '');
      ...
      var nvBadge = nv.overall_verdict ? P09Tab._verdictBadge(nv.overall_verdict, nv.papers_found) : '';
      var rtBadge = '';
      ...
      html += '<div class="stat-card cursor" onclick="P09Tab.showDetail(' + i + ')" style="flex:1 1 300px;min-width:280px;max-width:360px;padding:14px;transition:box-shadow .2s">'
  ...
  showDetail(idx) {
    var cands = App.DATA.p09_research_plans.candidates.slice().sort(function(a, b) { return b.final_score - a.final_score; });
    var cd = cands[idx];
    if (!cd) return;
    ...
    var noveltyHTML = P09Tab._renderNoveltySection(cd);
    var redteamHTML = P09Tab._renderRedteamSection(cd);
    ...
    if (cd.critique_text) {
      ...
      if (cd.detailed_feedback && Object.keys(cd.detailed_feedback).length) {
        ...
        for (var fi = 0; fi < fbKeys.length; fi++) {
          var flabel = P09Tab.DIM_LABELS[fbKeys[fi]] || fbKeys[fi];
  ...
  _renderNoveltySection(cd) {
    ...
    html += '<div class="field" style="font-size:12px;margin-bottom:8px">'
      + '整体判定: ' + P09Tab._verdictBadge(nv.overall_verdict, nv.papers_found)
    ...
    if (nvi.overall_verdict && nvi.overall_verdict !== nv.overall_verdict) {
      html += '<div style="font-size:11px;color:var(--text-muted);margin-bottom:8px">初始方案判定: ' + P09Tab._verdictBadge(nvi.overall_verdict, nvi.papers_found) + '（refine 前）</div>';
    }
    ...
    for (var vi = 0; vi < verdicts.length; vi++) {
      ...
      html += '<details class="field" style="margin-bottom:6px"><summary style="cursor:pointer;font-size:12px">'
        + P09Tab._verdictBadge(vd.verdict) + ' <span title="' + claim.replace(/"/g, '&quot;') + '">' + claimShort + '</span>'
